...
func _receive_net(steamid, data):
	# dont process packets from muted players
	var steamid_int = int(steamid)
	if PlayerData.players_muted.has(steamid_int):
		return
	var type: String = (data["type"]).trim_prefix("reelchat_")
	match type:
		"voice_packet":
			if int(get_parent().owner_id) == steamid_int:
				process_voice_data(data.voice_data, steamid_int)

func _ready():
	print("reelchat init")
	get_tree().root.get_node("LPSharccReelerChat").connect("reelchat_voice", self, "_receive_net")
...
func process_voice_data(voice_data: Dictionary, steamid: int)->void :
	get_sample_rate()
